...
from skrl_testing.utils.image_utils import save_image
# from skrl_testing.utils.isaaclab.direct_rl_env import DirectRLElleEnv
...
class CartpoleEnv(DirectRLEnv):
    ...
    def get_num_observations(self):
        self.num_img_observations = (
            self.cfg.tiled_camera.height, self.cfg.tiled_camera.width, self.cfg.frame_stack * 3
        )

        match self.cfg.obs_type:
            case "gt":
                return {'prop': self.cfg.num_gt_observations}
            case "image":
                return {'pixels': self.num_img_observations}
            case "image_prop":
                return {'pixels': self.num_img_observations, 'prop': self.cfg.num_gt_observations}
    ...
    def _configure_gym_env_spaces(self):
        ...
        if self.cfg.obs_type == "gt":
            self.single_observation_space = gym.spaces.Dict(
                {"prop": gym.spaces.Box(low=-np.inf, high=np.inf, shape=(self.num_observations['prop'],))}
            )
        elif self.cfg.obs_type == "image":
            self.single_observation_space = gym.spaces.Dict(
                {"pixels": gym.spaces.Box(low=-np.inf, high=np.inf, shape=(self.num_observations['pixels']))}
            )
    ...
    def _get_images(self):
        ...
        save_image(img_batch, img_name=f"cartpole_obs_n{self.num_envs}.png", nchw=False)
